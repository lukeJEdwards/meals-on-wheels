<template>
  <div class="base">
    <div class="titles">
      ...
      <div class="title pc">Postcode</div>
    ...
    <div class="info">
      ...
      >
        ...
        <p class=" address">{{ client.postcode }}</p>
        <div class=" tag-group" v-if="TooSmall">
          <p v-for="(tag, i) in client.comments" :key="i" class="tag">
            {{ tag }}
          </p>
        </div>
        <div class=" tag-group" v-if="TooSmall">
          <p
            v-for="(tag, i) in client.dietaryRequirements"
            :key="i + 100"
            class="tag"
          >
            {{ tag }}
      ...
      this.$http
        ...
        .then(Response => {
          Response.data.forEach(client => {
            this.$http.get(`/clients/${client._id}`).then(Response2 => {
              this.clients.push(Response2.data);
            });
          });
        })
        .catch(e => {
